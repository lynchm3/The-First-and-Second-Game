...
public class BodyOfWater {
	...
	public BodyOfWater(Square... squares) {
		for (Square square : squares) {
			...
			if (!square.inventory.containsGameObjectOfType(Templates.WATER_BODY.getClass()))
	...
	public BodyOfWater(int x1, int y1, int x2, int y2) {
		for (int i = x1; i <= x2; i++) {
			for (int j = y1; j <= y2; j++) {
				if (!Game.level.squares[i][j].inventory.containsGameObjectOfType(Templates.WATER_BODY.getClass())) {
