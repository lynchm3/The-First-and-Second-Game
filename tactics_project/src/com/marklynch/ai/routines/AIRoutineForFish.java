...
public class AIRoutineForFish extends AIRoutine {
	...
	public void update() {
		...
		if (state == STATE.SWIMMING) {
			if (targetSquare == null || targetSquare == fish.squareGameObjectIsOn
					|| !targetSquare.inventory.contains(WaterBody.class)) {
				targetSquare = AIRoutineUtils.getRandomSquare(1, 1, false, false);
				targetOffsetX = new Random().nextInt((int) (Game.SQUARE_WIDTH - fish.width));
				targetOffsetY = new Random().nextInt((int) (Game.SQUARE_HEIGHT - fish.height));
			}
			...
			new ActionFishSwim(fish, targetSquare, targetOffsetX, targetOffsetY).perform();
		}
	}
