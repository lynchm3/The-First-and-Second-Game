...
//	private Audio modStream;
    ...
    */
    public void init() {
	    ...
	    // a sound
	    oggEffect = AudioLoader.getAudio("OGG", ResourceLoader.getResourceAsStream("res/0477.ogg"));
 
	    // or setting up a stream to read from. Note that the argument becomes
	    // a URL here so it can be reopened when the stream is complete. Probablyw
	    // should have reset the stream by thats not how the original stuff worked
//	    oggStream = AudioLoader.getStreamingAudio("OGG", ResourceLoader.getResource("res/0477.ogg"));
...
//	    aifEffect = AudioLoader.getAudio("AIF", ResourceLoader.getResourceAsStream("testdata/burp.aif"));
	    ...
	    // a sound
	    wavEffect = AudioLoader.getAudio("WAV", ResourceLoader.getResourceAsStream("res/0477.wav"));
        } catch (IOException e) {
	    e.printStackTrace();
	}
    }
